function tempPlotMCADTorqueMap(MotorCADFEA,Input)
    ...
    % lambda_d = FEAdata.flux.d';
    % lambda_q = FEAdata.flux.q';
    EMTorque=MotorCADFEA.Electromagnetic_Torque;
    ...
    %Visualize the Torque surface
    figure(5);
    % mesh(id,iq,EMTorque);
    % hold on
    % mesh(id,iq,MotorCADFEA.TorqueDq,'EdgeColor','k');
    surf(MotorCADFEA.Id_Peak,MotorCADFEA.Iq_Peak,EMTorque);
    hold on
    scatter3(MotorCADFEA.Id_Peak,MotorCADFEA.Iq_Peak,MotorCADFEA.TorqueDq,'k');
    ...
    TorqueDq_new = interp2(id',iq,MotorCADFEA.TorqueDq',id_new',iq_new,'spline');
    ...
    figure(6);
    hold on
    mesh(id_new,iq_new,TorqueDq_new);
    scatter3(MotorCADFEA.Id_Peak,MotorCADFEA.Iq_Peak,MotorCADFEA.TorqueDq,'k');
    xlabel('I_d [A]')
    ylabel('I_q [A]')
    title('TorqueDq_new{Interp}'); grid on;
