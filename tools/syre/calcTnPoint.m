...
%
% ──────────────────────────────────────────────────────────────────────────────
% [JECT PATCH]  eMach/tools/syre/calcTnPoint.m
%   이 파일은 syre_public/mfiles/syreMMM/calcTnPoint.m 을 eMach genpath()로
%   오버라이드(shadow)하기 위한 패치 버전입니다.
%   원본 대비 변경 내용 (PATCH 주석 표시):
%     1. switch axisType: 'dq' case 추가 ('PM'과 동일 로직 — 감자축 d축 규약)
%     2. fTmp/iTmp 단조성(monotonicity) 보장 후 interp1 호출
%        (bilinear 보간으로 생기는 미세 진동으로 non-monotonic 발생 방지)
% ──────────────────────────────────────────────────────────────────────────────
...
if strcmp(ACSsafeFlag,'Yes')
    ...
    switch motorModel.data.axisType
        ...
        case {'PM','dq'}  % [PATCH] 'dq' same as 'PM'
            iTmp = unique(Id);
...
elseif ~isnan(limIso)
    ...
    out.Pfes_h = IronLossFactor*interp2(Id,Iq,Pfes_h,id,iq);
    out.Pfes_c = IronLossFactor*interp2(Id,Iq,Pfes_c,id,iq);
    out.Pfer_h = IronLossFactor*interp2(Id,Iq,Pfer_h,id,iq);
    out.Pfer_c = IronLossFactor*interp2(Id,Iq,Pfer_c,id,iq);
...
% [PATCH] 'dq' case 추가: demagnetizing d-axis PM convention (axisType='PM'과 동일)
% [PATCH] interp1 호출 전 fTmp 단조성 보장 (bilinear 보간 오차로 인한 non-monotonic 방지)
switch motorModel.data.axisType
    case 'SR'
        iTmp = unique(Iq);
        fTmp = interp2(Id,Iq,Fq,min(abs(Id(:)))*ones(size(iTmp)),iTmp);
    case {'PM','dq'}    % [PATCH] 'dq' = 감자축 d축 규약 (PM과 동일)
        iTmp = unique(Id);
        fTmp = -interp2(Id,Iq,Fd,iTmp,zeros(size(iTmp)));
        iTmp = -iTmp;
    otherwise
        % 알 수 없는 axisType: IHWC를 NaN으로 처리
        out.IHWC = NaN;
        fTmp = [];
end
