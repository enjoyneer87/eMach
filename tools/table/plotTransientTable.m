...
% table2Plot=tempTargetTable
% table2Plot=TablesCellCaseRowDataCol{1,2}{1}
% table2Plot=tempTargetTable.Variables
%%
    if istimetable(table2Plot)
        xTime = timerange(table2Plot.Time(1),table2Plot.Time(end),"closed");
    ...
    elseif istable(table2Plot)
        ...
        for i = 1:width(table2Plot)
            ...
            grphLineObj{i}=plot(xTime(1:end),table2Plot.(varName), 'DisplayName', strrep(varName, '_', ' '));
            hold on;
            end
            % Y축 라벨 설정: 변수 이름과 단위를 포함
            % ylabel(['[', varUnit, ']']);
        ...
        ElectricalAngle=[0:360/(len(table2Plot)-1):360];
        for i = 1:width(table2Plot)
        grphLineObj{i}=plot(ElectricalAngle',table2Plot);
        ax=gca;
        ax.XLim=[0 360];
        hold on;
        end
        xlabel('Electrical Angle [deg]'); % X축 라벨은 시간으로 가정
