...
PLAN = {'today': '2026-09-26',
 'next': '09-26 후속: 초기 정상상태 수렴 방식 확인과 10초 유지 4회 검사를 완료했다. 실제 Value 모드 강화로 내부 유체 경계 변화는 줄었지만 전체 약 0.0275°C drift는 남았다. '
         '다음은 열물성·R/P 갱신을 분리하고 C-low 손실/상태를 연결한다. 인터뷰는 추후 일괄, Overleaf Pull은 사용자가 수행한다.',
 'items': [{'pri': 'P1',
            'title': '7장 후속: 열물성·열저항 갱신과 C-low 손실 연결',
            'why': '초기 정상상태 수렴 검사 완료. Value 0.005°C/0.00005°C에서 반복 수 8/10회, 두 온도 이력 최대 차이 0.0003493°C. 내부 유체 경계는 안정됐지만 '
                   '권선·절연체의 약 0.0275°C drift는 남았다.',
            'next': '엄격한 Value 초기 상태에서 물성 평가 주기와 R/P·내부 스프레이 경계 갱신을 분리하고 치–절연체 경로를 확인한다. C-low 155 N·m·3860 rpm 손실 입력과 '
                    '상태 승계를 연결한다. 냉각 사양은 후속 인터뷰에서 결정한다.',
            'out': '과도 열망의 부하·온도 연동 검증',
            'owner': 'PC1',
            'effort': '후속 검증',
            'status': 'ready',
            'refs': [['https://drive.google.com/file/d/1cLjGX4i56XUg8xYFG620bLLMjT7xMLVX/view', '', '초기 정상상태 수렴 검사'],
 ...
 'other': [['완료',
           ...
           ['P1',
            '7장 시스템 코디자인',
            '고정 DAE·실제 Motor-CAD 과도 대조와 초기 수렴 검사 완료. 다음은 약 0.0275°C 잔여 drift의 열물성/R/P 갱신 분리와 C-low 입력 대응. 두 Value 설정의 '
            '이력 차이 0.0003493°C. 입구 온도·유량·냉각 질량/보조동력은 후속 인터뷰.'],
           ['P2',
            '기존 코디자인 M2 진단',
