...
PLAN = dict(
    today="2026-09-24",
    next=("원고 인터뷰 7회차로 위상 여유 개념을 다시 확인하고(6회차까지 두 번 비움) 9.7절 본문을 쓴다. 이어서 8.7·9.8 소결과 "
          "그림 PDF를 만든다. 사용자는 Codex 작업 뒤 이어서 진행한다(09-24). Overleaf 반영은 사용자 확인 뒤."),
    items=[
        dict(pri="P1", title="제4부 '구동 제어와의 연계' 원고 (인터뷰 형식)",
             why="09-24 골격과 본문 대부분을 썼다(커밋 55d0324): 8.1–8.6, 9.1–9.6, 1장 C5·4부 10장, 2·4장 연결, 결론 10장 기여·한계. "
                 "원고는 절마다 객관식 인터뷰로 사용자 이해를 확인한 뒤 쓴다(Thesis_SKKU defense/, 1–6회차 24문항).",
             next="인터뷰 7회차(위상 여유 재확인) → 9.7절(노치 속도 범위·위상 여유) 본문. 8.7·9.8 소결. 그림 PDF(8장 5장, 9장 7장)를 "
                  "figure/ch8·ch9에, 1장 thesis_flow에 제4부 상자. Overleaf 반영은 사용자 확인 뒤.",
             out="8·9장 완성 원고, 그림", owner="PC1 (인터뷰는 사용자)",
             effort="남은 초안 2–3일", status="ready",
             refs=[["gamma_wall_explainer.html", "j", "결과 4.9절"], ["drive_model_study.html", "harm", "해설 14절"]]),
        dict(pri="P1", title="저속 노치 과도 시험 (계단 응답)",
             why="노치 속도 범위의 정상상태는 확인했다(6–16 krpm ±2 %). 대가인 위상 여유 감소는 정상상태에 드러나지 않는다: 전류 "
                 "루프 계산으로 위상 여유 47° → 30°(6 krpm 노치), 1 A 계단 최대 1.25 → 1.41, 1.8 kHz 진동이 오래 남는다.",
             next="Simscape 2e에 토크(전류) 지령 계단을 넣어 6·8·10 krpm에서 노치 유무를 비교하고, 켜는 문턱(잠정 약 8 krpm)을 확정한다.",
             out="9.7절 근거, 켜는 문턱", owner="PC1", effort="2–3 h (분리 실행)", status="ready",
             refs=[["gamma_wall_explainer.html", "rem", "결과 4.9.8절"]]),
        dict(pri="P2", title="해설 보고서에 속도 범위·노치 설명 반영",
             why="결과 4.9.8절과 해설 14.2절은 16 krpm만 담고 있다. 09-24 속도 범위 결과와 설명 그림 5종(같은 전류, 여유 대 속도, "
                 "g 대 A, 노치 응답, 위상 여유)이 아직 보고서에 없다.",
             next="make_gamma_wall_explainer.py 4.9.8절과 make_drive_model_study.py 14.2절에 그림과 문단을 넣고 Drive에 다시 올린다.",
             out="보고서 갱신", owner="PC1", effort="1 h", status="ready",
             refs=[["gamma_wall_explainer.html", "rem", "결과 4.9.8절"]]),
    ...
    part=dict(
        title="제4부 — 구동 제어와의 연계 (09-24 확정·신설, 원고 진행 중)",
    ...
    closed={"기준 모델의 스큐 여부 확인": "사용자 확인: 실제 기계에 스큐 없음, 결론 크기 그대로",
            ...
            "노치의 속도 범위 확인 (16 krpm 밖)": "완료 09-24: Simscape 24건 + 조화균형, 노치 6–16 krpm 전 구간 ±2 %, 교차 위상 지연 "
                                            "6k −21.8° … 16k −4.1° → '저속 노치 과도 시험'으로 이어짐",
    ...
    sched=[
        ...
        ["2026-09-24", "2026-10-10", "제4부 8·9장 원고 (인터뷰)", "track"],
        ["2026-09-28", "2026-10-02", "저속 노치 과도 시험", "track"],
        ["2026-10-07", "2026-10-16", "P2 검증 보강", "track2"],
    ],
    decisions=[
        "Overleaf 반영: 제4부 신설과 장 번호 변경(결론 8 → 10장)을 Overleaf에 올려도 되는가(sync_overleaf.sh).",
        "노치 켜는 문턱: 잠정 약 8 krpm부터 — 저속 과도 시험 뒤 확정.",
        "제4부 그림: PNG 원본을 기존 장과 같은 규격의 PDF로 다시 뽑아 figure/ch8·ch9에 둔다(README_FIGURES 규약).",
    ],
)
