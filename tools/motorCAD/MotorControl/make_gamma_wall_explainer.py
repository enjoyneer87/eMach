...
<li><b>스큐:</b> 기준 모델은 스큐 없이 계산된 것으로 보인다(<code>SkewType = 0</code>). 3단 스텝 스큐 배열(±2.81° 기계각 = ±11.25° 전기각)은 정의만 되어 있다. 이 배열이 켜져 있다면 토크 12차는 (1 + 2 cos 135°)/3 ≈ −0.14배로 줄어든다. 실제 기계의 스큐 여부가 아래 결론의 크기를 좌우하므로 확인이 필요하다.</li>
</ul>
...
<p class="mut">한계: 제어기 구조(비례 이득, 디커플링 방식, 지연, 원형 클램프, 역산 안티와인드업)를 계산기에 그대로 적어 넣어야 한다. 노치·공진 제어기나 과변조처럼 다른 구조는 그 구조를 새로 넣어야 한다. PWM 측대파와 데드타임 고조파는 넣지 않았다. 기준 모델의 스큐 여부(4.9.2절)는 입력인 위치별 맵을 바꾼다.</p>
...
<tr><td>슬롯 고조파</td><td>제어기가 고조파 전류에 반응 → 클램프에서 잘림 → 평균 전류 오차</td><td class="bad">{pm(err("fea_pos", "MCB", 20), 0)} % / {pm(err("fea_pos", "MBC 95", 20), 0)} %</td><td>큰 여유에서만 ({nm_txt("fea_pos", 20)})</td><td><b>피드백의 6·12차 노치로 해결</b>(여유 5 %로 복귀, 비용 없음, 4.9.8절). 스큐 확인</td></tr>
...
<li><b>열린 문제:</b> 기준 모델의 스큐(적용되면 12차가 약 0.14배로 준다), Motor-CAD 전압 출력이 |R i + jω<sub>e</sub>ψ|보다 2–4 % 높은 원인, 노치를 속도에 따라 옮기는 구현과 16 krpm 밖(다른 속도·토크)에서의 확인.</li>
</ol></div>
