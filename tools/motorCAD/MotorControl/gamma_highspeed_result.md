...
### B0.3 2026-09-17 정정 두 건 (B1 이후 전 수치에 반영)

**(1) 상전압 한계는 285.1 V rms 다.** 인계 문서 2.1 절의 415.7 V 는 **피크**값(= 720/√3)인데 PC1 이 한동안
rms 로 썼다. Motor-CAD Lab 이 실제로 내놓은 해가 16/8/4 krpm 모두 **285.1 V rms** 에서 잘린다
(= 720 V × 0.97 / √3 / √2, 버스 이용률 97 %). 가용 영역이 좁아지므로 "진각이 안 움직인다"는 결론은 강화된다.

**(2) 토크 등고선의 교차점을 하나만 봤다.** γ 가 90° 에 가까우면 T(I) 가 단조가 아니라 교차가 둘인데
첫 교차(저전류, 전압 초과)만 잡고 끝나 "해 없음"으로 오판했다. 두 버그를 고쳐 전 분석을 다시 돌렸다.

정정 후 값: 16 krpm 가용 최대 토크 79.0 N·m(가능 최소 진각 86.4°), 8 krpm 156.8, 4 krpm 449.9 N·m.
Δγ 는 16·8 krpm 0.00°, 4 krpm 에서 Hybrid 0.00~+0.13° / Full-FEA 0.00~**+1.24°**, 손실 벌점 ≤ **0.014 %**.
실용 운전창(4 절 몬테카를로, 200 A rms)은 **86~88°** 로 좁아진다.

**(3) 플랜트 데이터는 Motor-CAD Lab export 로 통일한다(사용자 지시).** SyR-e 규약 dq맵은 d축 인덕턴스가
Lab 모델과 40 % 어긋나(특성전류 137 A rms 대 193 A rms) 앵커 5·6 이 어긋났다. Lab 의
*Saturation and Loss Maps*(Input Definition = D/Q, Calculation Method = Interpolate Lab Model)로
51×51 격자(Id −650~0, Iq 0~650 Apk)를 속도별 9 초에 뽑는다 — 자속·인덕턴스·토크·철손(고정자/회전자,
히스테리시스/와전류)·교류 동손·자석손이 결측 0 % 로 한 파일에 나온다.
스크립트 `e10drive/export_lab_satmap.py`, 정리·앵커 `e10drive/prep_e10_maps_lab.m`.
