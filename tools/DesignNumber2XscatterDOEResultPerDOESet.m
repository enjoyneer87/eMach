function [legend_colorsh1,legend_colorsh2]=DesignNumber2XscatterDOEResultPerDOESet(DOEResult, color, legendName, useFilled, varargin)
    DOE1 = DOEResult;
    ...
    for i = 1:length(Fieldlist1)
        ...
        if ~isempty(DOE1.(FieldName)) && isfield(DOE1.(FieldName), 'Weight') && isfield(DOE1.(FieldName), 'SumofTotalLoss')
            ...
            if useFilled==1
                h1 = scatter3(str2num(strrep(FieldName,'Design','')),o_Weight_Act, SumOfTotalLoss, 'Marker', customMarker,'MarkerEdgeColor', color, 'MarkerFaceColor',color, 'DisplayName', FieldName, 'LineWidth', 2);
            else
                h1 = scatter3( str2num(strrep(FieldName,'Design','')),o_Weight_Act, SumOfTotalLoss,'Marker', customMarker,'MarkerEdgeColor',color, 'DisplayName', FieldName, 'LineWidth', 2);
            end
            hold on;
    ...
    for i = 1:length(Fieldlist1)
        ...
        if ~isempty(DOE1.(FieldName)) && isfield(DOE1.(FieldName), 'Weight') && isfield(DOE1.(FieldName), 'SumofTotalLoss')
            ...
            % 'useFilled' 옵션이 제공되었는지 확인하고 마커 표면을 채움
            if useFilled==1
                h2 = scatter3(str2num(strrep(FieldName,'Design','')),TotalMotorGearWeight, SumOfTotalLoss,'Marker', customMarker,'MarkerEdgeColor', color, 'MarkerFaceColor',color,  'DisplayName', FieldName, 'LineWidth', 2);
            else
                h2 = scatter3(str2num(strrep(FieldName,'Design','')),TotalMotorGearWeight, SumOfTotalLoss,'Marker', customMarker, 'MarkerEdgeColor', color,  'DisplayName', FieldName, 'LineWidth', 2);
            end
            hold on;
