...
if istable(PartStruct)
    ...
    for Partindex=1:len(WireIndexList)
        ...
        NodeCoord        =PartStruct.NodeTable{Partindex}.nodeCoords;
        ...
        if width(elementConnectivityMat)>3
            PartStruct.DT{Partindex}=delaunayTriangulation(PartStruct.DT{Partindex}.Points);
        else
            PartStruct.DT{Partindex}=triangulation(elementConnectivityMat,PartStruct.DT{Partindex}.Points);
        end
