function saveNodeID2Mat(NodeID, jplot_file_path)
    ...
    mat_file_path = fullfile(folder, [name, 'NodeID.mat']); % .mat 파일 경로 생성
    
    % partIDs 변수를 .mat 파일로 저장
    try
        save(mat_file_path, 'NodeID','-v7');  % partIDs라는 이름으로 .mat 파일에 저장
        fprintf('NodeID successfully saved to %s\n', mat_file_path);
    catch ME
