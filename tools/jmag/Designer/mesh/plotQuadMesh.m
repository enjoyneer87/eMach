function plotQuadMesh(nodeId, nodes, centerPoints, numNodes)
    ...
    %
    % 이 함수는 마름모 또는 사각형 패치를 생성하고 시각화합니다.
    ...
    % nodeId를 기준으로 nearestNodes의 인덱스를 변환하여 nodeCoords로 변환
    vertexIndices = nearestNodes;  % numCenters x numNodes 크기의 행렬 (인덱스)

    % patch를 한 번에 실행: Faces와 Vertices 설정
    figure;
    hold on;

    % 'Vertices'는 모든 노드 좌표, 'Faces'는 각 요소의 노드 인덱스
    patch('Faces', vertexIndices, 'Vertices', nodes, 'FaceColor', 'cyan', 'EdgeColor', 'black');

    % 전체 노드 좌표 표시
    plot(nodes(:, 1), nodes(:, 2), 'ro');  % 전체 노드를 빨간 점으로 표시
    title('Quadrilateral or Rhombus Mesh');
    axis equal;
    hold off;
end