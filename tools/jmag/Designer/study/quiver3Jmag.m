function quiver3Jmag(tr,elementCurl)
% 삼각형 구성 정보 (요소)
...
quiver3(centroids(:, 1), centroids(:, 2), zeros(size(centroids, 1), 1), ...  % 벡터 시작점
        elementCurl(:,1), elementCurl(:,2), elementCurl(:,3), ...  % 벡터 방향 (z 방향으로만 표시)
        'AutoScale', 'on', 'LineWidth', 2);  % z 방향으로 크기를 elementCurl 값에 비례하게 표시
