function PartStruct = getEdgeVertexIdwithXYZCheck(PartStruct, app)
    ...
    for partIndex = 1:length(PartStruct)
        ...
        for IdIndex = 1:height(PartStruct(partIndex).Vertex.VertexIds)
            ...
            tempPositionTable = mergevars(tempPositionTable, {'x', 'y', 'z', 'r', 'theta'}, 'NewVariableName', 'VertexPosition', 'MergeAsTable', false);
        ...
        for IdIndex = 1:height(PartStruct(partIndex).Edge.EdgeIds)
            ...
            tempEndPosTable = array2table(EndPos, "VariableNames", {'x', 'y', 'z', 'r', 'theta'});
            ...
            tempEndPosTable = mergevars(tempEndPosTable, {'x', 'y', 'z', 'r', 'theta'}, 'NewVariableName', 'endPosition', 'MergeAsTable', false);
            % Table row
            StartPosTable = [StartPosTable; tempStartPosTable];
            MidPosTable = [MidPosTable; tempMidPosTable];
            EndPosTable = [EndPosTable; tempEndPosTable];
        end
    
        %% getEdge - VertexId with XYZCheck 
        EdgeNumber = height(PartStruct(partIndex).Edge);
    
        PartStruct(partIndex).Edge = [PartStruct(partIndex).Edge StartPosTable MidPosTable EndPosTable];
        ...
        for EdgeIndex = 1:height(PartStruct(partIndex).Edge)
            matchingRowinVertexWithStartPos = ismember(PartStruct(partIndex).Vertex.VertexPosition, PartStruct(partIndex).Edge.startPosition(EdgeIndex, :), 'rows');
            matchingRowinVertexWithMidPos = ismember(PartStruct(partIndex).Vertex.VertexPosition, PartStruct(partIndex).Edge.midPosition(EdgeIndex, :), 'rows');
            matchingRowinVertexWithEndPos = ismember(PartStruct(partIndex).Vertex.VertexPosition, PartStruct(partIndex).Edge.endPosition(EdgeIndex, :), 'rows');
        ...
        % 
        % if ~isempty(PartStruct(partIndex).Vertex)
        PartStruct(partIndex).VertexMaxThetaPos = max(PartStruct(partIndex).Vertex.VertexPosition(:, 5));
        PartStruct(partIndex).VertexMinThetaPos = min(PartStruct(partIndex).Vertex.VertexPosition(:, 5));
        PartStruct(partIndex).VertexMaxRPos = max(PartStruct(partIndex).Vertex.VertexPosition(:, 4));
        PartStruct(partIndex).VertexMinRPos = min(PartStruct(partIndex).Vertex.VertexPosition(:, 4));
       % end
    end
% 
