...
ActiveXParametersStruct=mcad.getMcadActiveXTableFromMotFile(ActiveXParametersStruct);
end
...
%% 크기별로 데이터 정리
nMapPoints = ModelBuildPoints_Current_Lab * ModelBuildPoints_Gamma_Lab;

% nI*nG 크기 배열 (Psi, Fe loss map 등) — 격자 크기에 무관하게 동작
height30Rows2Table = filterMCADTableWithSize(mcadTable4TxtValues, nMapPoints, 1);

% 스칼라 파라미터 (nI*nG 아닌 것 중 크기 1인 것)
height1Rows2Table  = filterMCADTableWithSize(mcadTable4TxtValues, 1, 1);

% 나머지: nI*nG도 아니고 스칼라도 아닌 배열 (AC loss 등 다른 크기)
