...
try:
    # ── 기하/재료/회로 파라미터 (실측) ───────────────────────────────────
    R_SHAFT, R_ROT_OUT = 0.022225, 0.065
    R_STA_IN, R_STA_OUT = 0.066, 0.099
    STACK, ROT_STACK = 0.160, 0.150
    R_COIL_IN, R_COIL_OUT = 0.067, 0.083
    Z_SHAFT_END = 0.1167
    ...
    LOSS_IRON_ROTOR, LOSS_IRON_STATOR = 59.7, 1145.8
    ...
    # ── 메시 임포트 ──────────────────────────────────────────────────────
    mapdl.cdread("DB", r"D:\KDH\simVary\Ansys_Thermal\real_motor_mesh", "cdb")
    ...
    N = {nm: net_node(i + 1) for i, nm in enumerate(
        ["WJ", "ATF", "AMB", "AIR", "SHF", "H_WJ", "H_ATF", "H_RST",
         "GAP_S", "GAP_R", "CEND_ATF", "CEND_AIR"])}
    _rid = [100]
    def add_C(node, c):
    ...
    add_C(N["SHF"], C_SHAFT_V); add_C(N["H_WJ"], C_HOUSING / 4)
    add_C(N["H_ATF"], C_HOUSING / 4); add_C(N["H_RST"], C_HOUSING / 2)
    add_C(N["AIR"], 10.0)
    # 코일엔드 열용량은 V_end 산출 후 아래 HGEN 단계에서 추가
    A_GAP = 2 * math.pi * ((R_ROT_OUT + R_STA_IN) / 2) * STACK
    edges = [
        ("GAP_S", "GAP_R", TC_AIR * A_GAP / 0.001),
        ("H_WJ", "WJ", G_WJ),
        ...
        # 코일엔드(lumped, JAC279 Coil end cooling): 표면적 근사 0.30 m2 (양단)
        ("CEND_ATF", "ATF", HTC_ATF * 0.30 * 0.25),   # ATF 사분면 h=300
        ("CEND_AIR", "AIR", HTC_AIR * 0.30 * 0.75),   # 나머지 h=10
    ]
    for a, b, g in edges:
        add_R(N[a], N[b], g)
    for nm in ("WJ", "ATF", "AMB"):
        mapdl.d(N[nm], "TEMP", T_INIT)
    P("circuit built")
    ...
    def sel_cyl(radius=None, th_ranges=None, z_range=None, mat=None, rt=None):
        ...
        return _fn
    # 스테이터 외경 3분할
    make_surf(sel_cyl(R_STA_OUT, TH_WJ, mat=M_CS), N["H_WJ"], H_CONTACT, "statorOD-WJ")
    make_surf(sel_cyl(R_STA_OUT, TH_REST, mat=M_CS), N["H_RST"], H_CONTACT, "statorOD-REST")
    make_surf(sel_cyl(R_STA_OUT, TH_ATF, mat=M_CS), N["ATF"], HTC_ATF, "statorOD-ATF")
    # 공극 양면
    ...
    def sel_coilcut(th_ranges):
        ...
        return _fn
    make_surf(sel_coilcut(TH_ATF), N["CEND_ATF"], H_CU_END, "coilcut-ATF")
    make_surf(sel_coilcut(TH_WJ + TH_REST), N["CEND_AIR"], H_CU_END, "coilcut-rest")
    ...
    V_WIND_TOTAL = 365.3e-6          # 절단 전 권선 총체적 (이전 메시 실측)
    V_slot = float(np.abs(_vols[_mats == M_CO]).sum())
    f_end = max(0.0, 1.0 - V_slot / V_WIND_TOTAL)
    Q_end = LOSS_COPPER * f_end
    Q_slot = LOSS_COPPER - Q_end
    P(f"coil: V_slot={V_slot*1e6:.1f} cm3, f_end={f_end:.2f}, "
      f"Q_slot={Q_slot:.0f} W, Q_end={Q_end:.0f} W")
    for mat, W in ((M_CS, LOSS_IRON_STATOR), (M_CR, LOSS_IRON_ROTOR),
                   (M_MG, LOSS_MAGNET), (M_CO, Q_slot)):
        vol = float(np.abs(_vols[_mats == mat]).sum())
        q = W / vol
        mapdl.allsel(); mapdl.esel("S", "MAT", "", mat)
        mapdl.esel("R", "TYPE", "", 1)
        mapdl.bfe("ALL", "HGEN", 1, q)
        P(f"  mat {mat}: V={vol*1e6:.1f} cm3 q={q:.3e} W/m3")
    mapdl.allsel()
    # 코일엔드 회로: 열용량(rho c V_end) + 손실 주입 (ATF 25% / 나머지 75%)
    V_end = V_WIND_TOTAL - V_slot
    C_end = 8960.0 * 0.45 * 385.0 * V_end
    _rid[0] += 1; mapdl.type(4); mapdl.real(_rid[0])
    mapdl.r(_rid[0], C_end * 0.25); mapdl.e(N["CEND_ATF"])
    _rid[0] += 1; mapdl.type(4); mapdl.real(_rid[0])
    mapdl.r(_rid[0], C_end * 0.75); mapdl.e(N["CEND_AIR"])
    mapdl.f(N["CEND_ATF"], "HEAT", Q_end * 0.25)
    mapdl.f(N["CEND_AIR"], "HEAT", Q_end * 0.75)
