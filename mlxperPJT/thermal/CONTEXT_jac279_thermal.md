...
### 4e. 권선 직교이방성 (2026-07-21)

참고: `2023_Anisotropic Thermal Conductivity for Electric Motor Coil-Windings.pdf`
(Ansys Fluent 워크샵). 권선은 소선 방향(축) k 高 / 횡 k 低 의 **직교이방성**.
논문 권장 homogenized 값 **(2.5, 2.5, 250) W/mK** (횡/횡/축).

- **핵심 단순화**: 코일엔드를 이미 절단·회로화(4d)했으므로 FEM 코일은 슬롯 직선부
  뿐 → 소선이 전부 **글로벌 z축**. 논문의 곡선좌표계(CCS) 불필요, SOLID87
  `MP,KZZ=250 / MP,KXX=MP,KYY=2.5` 직접 사용(ESYS 회전 불요).
  검증: 1×1×1 블록 x구배 k_eff=2.50, z구배 k_eff=250.0 정확 일치.
- 코일엔드 구리전도 h 도 `K_COIL_AXIAL/0.055` 로 일관화(6900→4545).
- **효과** (등방성 380 → 이방성): 횡 k 380→2.5(≈150배↓)로 슬롯에서 코어로 나가는
  전도 병목 → **코일 max 177.2 → 192.0°C (+14.9K)**, 슬롯 내부 온도폭 33.7→47.0K.
  각 도체 단면에 **횡방향 hotspot**이 생김(등방성은 도체별 균일 → 이 구배를 놓침).
  스테이터/자석은 거의 불변(전도율 병목 아님). **192°C 는 H급 절연 180°C 초과** —
  등방성 모델이 가리던 마진 문제. `coil_iso_vs_aniso_z0.png` 로 직접 대조.
- 결과·비교 시각화: `viz_real_aniso/` (나란히 단면, 슬롯 확대, 부품 이력).
  기존 등방성 `viz_real/` 는 보존.
- ⚠ 축 k=250 은 논문값(FILL~0.6 상당). 우리 FILL=0.45 자기일관값은 ~180.
  둘 다 `K_COIL_AXIAL` 파라미터로 조정 가능(횡 k 가 지배적이라 축값 영향 작음).
