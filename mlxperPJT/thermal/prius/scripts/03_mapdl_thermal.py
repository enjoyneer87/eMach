...
log = open(os.path.join(SP, "run_prius.txt"), "w", encoding="utf-8")
def P(*a):
...
try:
    ...
    # Prius 2D Maxwell 손실 (full 360, 83.8mm, 250A/3000rpm/gamma60)
    LOSS_COPPER_SLOT = 2311.7   # 슬롯 동손 (2D=슬롯분, FEM HGEN)
    LOSS_COPPER_END = 1038.7    # 엔드 동손 (슬롯×V_end/V_slot=0.449, 회로)
    LOSS_MAGNET = 23.8
    LOSS_IRON_ROTOR, LOSS_IRON_STATOR = 65.0, 584.9
    ...
    Q_slot = LOSS_COPPER_SLOT       # 2D 슬롯 동손 직접
    Q_end = LOSS_COPPER_END         # 엔드 동손 (형상 비율로 산정)
    P(f"coil: V_slot={V_slot*1e6:.1f} cm3, Q_slot={Q_slot:.0f} W, "
      f"Q_end={Q_end:.0f} W (Prius 2D)")
    ...
    V_end = V_slot * 0.449          # 엔드/슬롯 체적비(형상)
