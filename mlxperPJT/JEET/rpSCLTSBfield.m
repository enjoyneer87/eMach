...
% load('wireTable_SCL_TS_9krpm_case2.mat');
...
for slotIndex=1:3:height(WireTable)
    ...
    yline(mean(sqrt(Slot4TTimeArray([outMostIndex],:).^2),2),'-.r',EleStrIdCell{slotIndex})
    ax=gca;
...
%% linePlot 
    ...
    for slotIndex=1:3:height(WireTable)
        ...
        EleStrIdCell{slotIndex} = cellfun(@(x) strcat('TS-FEA',x),elePos{slotIndex}, 'UniformOutput', false);
        % newLegend=[newLegend EleStrIdCell{slotIndex}]
        yline(mean(sqrt(Slot4RTimeArray([outMostIndex],:).^2),2),'-.r',EleStrIdCell{slotIndex})
        ax=gca;
        ax.Legend.String=EleStrIdCell{slotIndex};
