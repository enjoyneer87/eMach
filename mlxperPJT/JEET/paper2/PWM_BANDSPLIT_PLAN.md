...
## 4e. M2 P4 실행 결과 (2026-08-28) — **통과(단, 속도 정정 필요)**

> ⚠ **2026-08-28 야간 정정**: 이 실험은 `AngularVelocity` 단위 오류로
> **959,000 r/min** 에서 돌았다(JMAG 은 rev/s, 2024 규약 `'speed/60'`).
> A·B 통제 비교라 중첩 판정(ratio 0.985~0.997)은 유지되나 **정격 운전점
> 서술은 성립하지 않는다** — 논문 반영 전 정격 속도 재실행 필요.
> P1~P3(FP-Fq 선형)은 회전 조건 미사용이라 무영향. 상세는 노트 §12.31.
