...
for PJTIndex=1:len(MQSList)
    ...
    ModelPath      =fullfile(PJTJfilesPath,subPath(ModelFolder).name);
    folderNameList=findFolderNames(ModelPath);
    folderNameList=folderNameList(~contains(folderNameList,'Fq','IgnoreCase',true));
    % StudyfolderNameList=cell(len(folderNameList),2);
    for subIndex=1:len(folderNameList)
        tempSubFolderNames=findFolderNames(fullfile(ModelPath,folderNameList{subIndex}));
        if ~isempty(tempSubFolderNames)
         StudyfolderNameList{subIndex,1}=tempSubFolderNames;
         StudyfolderNameList{subIndex,2}=fullfile(ModelPath,folderNameList{subIndex});
        end
    end
    end
