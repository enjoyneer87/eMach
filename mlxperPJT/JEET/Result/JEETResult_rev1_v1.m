%% [TB]???뚯씪濡????띾룄?먯꽌 ?쒕떎?뚯뿉 ?뚯씪??留뚮뱾?댁꽌 ?띾룄蹂꾨줈 ?섎뒗嫄몃줈 蹂寃?%% MCAD GetData


rpm = 8000

% dev_e10_ACLoss - Branch - ??젣?좉쾬
...
colors = lines(numFaces); % 媛?Face??????ㅻⅨ ?됱긽???앹꽦?⑸땲??
figure;
pdemesh(model)
x = pdeNodes(1,:);
y = pdeNodes(2,:);
...
set(line_handles, 'Color', [0.8,0.8,0.8]); % ?? 寃???%% 
% Fig 9 MVP Contour
...
b= 5.30333
bm=mm2m(b)
bc = mm2m(REFdimensions(1)); % ??[m]
hc = mm2m(REFdimensions(2)); % ?믪씠 [m]
...
SLCB= 5.30333*2
SLCbm=mm2m(SLCB)
SCLbc = mm2m(SCLdimensions(1)); % ??[m]
SCLHC = mm2m(SCLdimensions(2)); % ?믪씠 [m]
...
% calc? mcad鍮꾧탳
% devCalcMCADHybridACLoss ?먯꽌 
%% 
% * ?⑥씪 B媛믪쑝濡?OuputData? 鍮꾧탳           - ?꾩옱 ?쇰떒 ?덈쭪??% * Graph媛믪쑝濡?鍮꾧탳 - outputData? 鍮꾧탳 : 
% * MCAD 洹몃옒??媛?몄삤湲?% * loadMCADSimulData濡?洹몃옒??B,AC) 遺덈윭?ㅺ린, ?닿굅湲곗??쇰줈 鍮꾧탳 - 1000*48 鍮꾩쑉?덈뒗???닿쾬???섏븞留욎쓬
% * jmag怨쇰룄 B媛?position?뚮Ц??2D濡??섎뜑?쇰룄 ?ㅻ쫫
%% 
% 
