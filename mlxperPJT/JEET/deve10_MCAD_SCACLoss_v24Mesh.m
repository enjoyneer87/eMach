...
% MeshSizeList  = [0.2:0.2:0.8];
MeshSizeList  = [0.2,0.8];
TimeStepList  = [60:60:360];
...
for TimeStepIndex=1:length(TimeStepList)
    mcad.SetVariable('TorquePointsPerCycle',TimeStepList(TimeStepIndex));
    for MeshIndex=1:length(MeshSizeList)
        mcad.SetVariable('StatorSlotMeshLength',MeshSizeList(MeshIndex));
        ...
        MeshSize  = ['Mesh',num2str(MeshIndex)];
        TimeSize  = ['Step',num2str(TimeStepList(TimeStepIndex))];
        MatFilePath           = fullfile(MatDir,['MatBy_',MfileName,'_',MeshSize,'_',TimeSize,'_',LoadInput,'.mat']);
