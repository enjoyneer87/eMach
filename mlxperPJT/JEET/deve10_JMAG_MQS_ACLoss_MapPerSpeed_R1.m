...
for PJTIndex=1:len(MQSList)
    ...
    addJMAGEqaution('SkinDepth','sqrt(2/(omegaE*diffusionFactor))*1000',curStudyObj)
    ...
    ParameterListTable(:,paraName{1}).Variables=cellstr(num2str([0:22.5:22.5*(height(ParameterListTable)-1)+0]'));
    paraName  =varName(contains(ParameterListTable.Properties.VariableNames,'speed'));
    % ParameterListTable(:,paraName{1}).Variables=cellstr(num2str([1000:6000:6000*(height(ParameterListTable)-1)+1000]'));

    % paraName  =varName(contains(ParameterListTable.Properties.VariableNames,'speed'));
    ParameterListTable(:,paraName{1}).Variables=repmat({'8000'},height(ParameterListTable),1);
...
%% Get Result
FqfilterName={'SCL_e10_WTPM_Pattern'};
