...
20. **커널 차원수 연구 (`conductor_je_2d` / `conductor_je_strips`).**
    ...
    **수정 후 결과** (슬롯1, 32스텝, 주기 RMS 평균제곱 비, TS 기준):
    ...
    | 커널 | 경계 샘플 수 | Ref | SC |
    |---|---|---|---|
    | 1-D, 막대당 2점 | 12 | 14.46 | 8.9 |
    | 1-D, 20 스트립 | 240 | 4.96 | 4.47 |
    | 2-D, 둘레 전체 | 768 | **2.81** | **1.43** |
    | (참고) 실제 AF | --- | 2.253 | 1.511 |

    **밀도와 차원이 둘 다 기여하며, 어느 하나가 단독 주범이 아니다.**
    2점 -> 스트립으로 밀도를 올리면 14.5 -> 5.0(Ref), 8.9 -> 4.5(SC)
    로 절반쯤 줄고, **차원까지 올려야** AF 수준에 도달한다.

    > ⚠ 한때 스트립이 2.65(Ref)/3.41(SC)로 더 좋아 보였는데, 그건
    > MS-FEA 의 **함침 포함** 영역(25% 넓음)에서 B 를 뽑아 면간 기울기가
    > 부풀려진 탓이었다. TS-FEA 의 진짜 구리 면으로 바로잡자 4.96/4.47
    > 로 내려앉았다. **2-D 만 기하 변경에 강건**했다(2.83->2.81,
    > 1.39->1.43) --- 기하가 의심스러울 때 어느 결과를 믿을지의 기준이
    > 된다.

    **그리고 2-D 잔차가 실제 AF 와 같은 크기다**(Ref 2.81 대 2.25,
    SC 1.43 대 1.51). 재구성은 MS-FEA 의 **와전류 없는** 장을 경계조건으로
    쓰므로, 커널을 아무리 올려도 반작용 기여에는 닿지 못한다 --- 논문
    2.2절 '첫째 위반'이 그것이며, Fig 2 는 이 점을 Ref/SC 두 스케일에서
    보인다.

    **재현 확인 (8 kRPM, 같은 전류·위상·슬롯):** 속도만 바꾸면 f_e 가
    절반이 되어 eta 는 1/sqrt(2) 로 줄지만 AF 는 오히려 커진다 --- 두
    양이 반대로 움직이므로 우연의 일치를 배제하는 시험이 된다.

    | case | eta | 1-D 2점 | 스트립 | 2-D | 실제 AF |
    |---|---|---|---|---|---|
    | Ref 16k | 0.75 | 14.46 | 4.96 | **2.81** | 2.253 |
    | Ref 8k | 0.53 | 16.96 | 2.79 | **3.28** | 2.705 |
    | SC 16k | 1.49 | 8.84 | 4.47 | **1.43** | 1.511 |
    | SC 8k | 1.06 | 11.13 | 5.11 | **2.02** | 1.719 |

    - **크기 순서 4/4 일치** (AF `[2 1 4 3]` = 2-D `[2 1 4 3]`)
    - **2-D / AF = 1.15 +- 10%** (1.247, 1.213, 0.946, 1.175)
    - 상관: 2-D 대 AF **+0.985**, 2-D 대 eta **-0.990**
    - 대조군 스트립은 AF 와 -0.706 (역상관), 비도 1.03~2.97 로 요동

    즉 2-D 잔차는 eta 가 아니라 **AF 를 따라간다**. 반작용 기여를 보고
    있다는 해석과 일치하며, 스트립은 그렇지 않아 대조군 역할을 한다.
    실행: `run_kernel_dim_study.py --figures --model {Ref8k,SC8k}`.
