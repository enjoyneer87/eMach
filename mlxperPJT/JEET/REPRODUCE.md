...
20. **커널 차원수 연구 (`conductor_je_2d` / `conductor_je_strips`).**
    같은 MS-FEA 여기(와전류 없는 `a_wbm` 을 막대 경계에 Dirichlet)를
    주고 커널만 바꿔 비교한다:

    - `hybrid_je_at_points` --- 1-D, 막대당 **경계 2점**
    - `conductor_je_strips` --- 1-D 이되 접선 방향 **N개 스트립**마다
      따로 풂 (Motor-CAD FEA Paths 방식 모사, 주의 22)
    - `conductor_je_2d` --- 막대 단면 **완전 2-D** (네 면 모두 사용)

    > ⚠ **초기 2-D 수치(TS/2-D = 1.00 등)는 폐기.** 격자 생성에서
    > `width_mm` 을 반경축에, `height_mm` 을 접선축에 넣어 **막대를
    > 90도 눕힌 채** 풀고 있었다. 얇은 쪽이 delta 대비 두께를 정하므로
    > 해가 완전히 달라진다(공극바 |Je|rms 165.2 -> 65.8 A/mm^2).
    >
    > **교훈**: 로컬 좌표계를 만들 땐 어느 축이 반경인지 **격자 반경
    > 범위로 검산**할 것. 수정 후 1.709 mm 로 `.mot` Copper_Height
    > 1.686 과 맞는다(눕혀졌을 땐 3.711 이 나왔어야 함). 검산은 한 줄인데
    > 안 했고, 4-패널 그림에서 막대가 슬롯 밖으로 어긋나 보여서야 발견됐다.

    **수정 후 결과** (슬롯1, 32스텝, 주기 RMS 평균제곱 비, TS 기준):
    ...
    **결론이 뒤집혔다: 크기 과소평가의 주범은 커널 차원수가 아니라
    샘플링 밀도다.** 2점 -> 20스트립만으로 14.5배가 2.65배로 줄고,
    완전 2-D 는 2.83 으로 오히려 근소하게 나쁘다. 즉 접선 방향으로
    B 를 여러 곳에서 뽑는 것이 결정적이고, 커널을 2-D 로 올리는 것은
    그 위에 추가 이득이 없다.

    다만 **공간 분포는 반대**다. (c) 스트립판은 스트립끼리 결합이 없어
    세로 줄무늬 인공물이 생기고 슬롯바닥 바가 과하게 밝다. (d) 2-D 는
...
22. **"Hybrid (reference)" 패널은 Motor-CAD 의 하이브리드가 아니다 ---
    실제 방법보다 불리하게 그려진다.** Motor-CAD FEA 화면(E-Magnetics
    > FEA / FEA Paths)에서 슬롯 영역(`StatorSlotR1`, 면적 81.92 mm^2)에
    **다수의 샘플링 라인**(막대당 ~20개 스트립)이 그어져 있는 것이
    확인된다. 즉 실제 하이브리드는 막대 하나에서 B 를 여러 지점 뽑아
    스트립별로 근접 손실을 계산하는 반면, 우리 재구성은 **막대당 경계
    2점**(안/바깥 면)만 쓴다.

    따라서 Fig 2 (b)의 과소평가에는 (i) 1-D 커널의 한계와 (ii) 우리
    재구성의 성긴 샘플링이 **둘 다** 섞여 있다. 워크플로 검증도 같은
    결론이었다(실제 하이브리드 대비 2~3배 더 낮게 표현). 논문에서
    (b)를 "Motor-CAD 하이브리드의 출력"으로 서술하면 안 되고,
    "동일 가정(1-D 확산 + MS-FEA 여기)에서의 참고 재구성"으로 한정할
    것 --- 현재 캡션은 그렇게 쓰여 있다.
