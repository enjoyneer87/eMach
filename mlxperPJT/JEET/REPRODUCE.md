...
20. **커널 차원수 연구 (`conductor_je_2d` / `conductor_je_strips`).**
    ...
    **교훈**: 로컬 좌표계를 만들 때 어느 축이 반경이고 어느 축이 접선인지
    를 **격자 반경 범위로 검산**할 것 --- 수정 후 격자의 실제 반경 범위는
    1.709 mm 로 `.mot` Copper_Height 1.686 mm 와 맞는다. 눕혀져 있을 땐
    3.711 mm 가 나왔어야 했는데 확인하지 않고 넘어갔다.
