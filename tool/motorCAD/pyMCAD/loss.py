...
Canonical implementation lives in `tools.motorCAD.pyMCAD.loss`.
This module remains so older notebooks importing `tool.motorCAD.pyMCAD.loss`
continue to work.
"""

from tools.motorCAD.pyMCAD.loss import *  # noqa: F401,F403
...
class ElementMesh:
    ...
    def from_magnetic_regions(cls, mr: MagneticRegions) -> "ElementMesh":
        ...
        return cls(node_xy=node_xy, elements=tuple(elements))
...
def interactive_loss_fields_plot(
    ...
    *,
    ...
    reg_text = widgets.Text(
        value="" if initial_reg_code is None else str(initial_reg_code),
    ...
    display(widgets.VBox([widgets.HBox([name_dd, reg_text, size_slider]), out]))
    _draw()
...
def interactive_scalar_field_plot(
    ...
    *,
    ...
    display(widgets.VBox([widgets.HBox([reg_text, size_slider]), out]))
    _draw()
